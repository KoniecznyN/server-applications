...
app.config['SECRET_KEY'] = 'gbnghj%^&*(*&TFGHJKKHGT^&*UYui8' # klucz szyfrowania danych z formularza
moment = Moment(app)
date = datetime.now()
...
def serverError(e):
    return render_template('500.html', title='Wewnętrzny błąd servera', userLogin=session.get('userLogin'), firstName=session.get('firstName')), 500
